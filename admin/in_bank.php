...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty'])&& $_POST['faculty'] === 'internal') {
        ...
        $stmt = $conn->prepare("SELECT in_acc_name,in_Id, REPLACE(in_acc_no, ',', '') AS in_acc_no
                                FROM exam 
                                WHERE date >= ?
                                 GROUP BY in_Id");
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sheet->setCellValue('B' . $rowIndex, $row['in_Id']);
                $sheet->setCellValue('C' . $rowIndex, $row['in_acc_name']);
                $sheet->setCellValueExplicit('D' . $rowIndex, $row['in_acc_no'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sumStmt = $conn->prepare("SELECT SUM(in_tot) AS total_amount FROM exam WHERE in_Id = ? AND date >= ?");
                $sumStmt->bind_param('ss', $row['in_Id'], $dateFrom); // Bind both parameters
                $sumStmt->execute();
    ...
    $totalRowNumber = $rowIndex + 1; // Assuming you have a total row
        ...
        header('Content-Disposition: attachment;filename="internal_bank.xlsx"');
