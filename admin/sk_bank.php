...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty'])&& $_POST['faculty'] === 'skilled_assistant') {
        ...
        $stmt = $conn->prepare("SELECT sk_acc_name, sk_Id, REPLACE(sk_acc_no, ',', '') AS sk_acc_no
                                FROM exam 
                                WHERE date >= ?
                                 GROUP BY sk_Id");
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sheet->setCellValue('B' . $rowIndex, $row['sk_Id']);
                $sheet->setCellValue('C' . $rowIndex, $row['sk_acc_name']);
                $sheet->setCellValueExplicit('D' . $rowIndex, $row['sk_acc_no'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sumStmt = $conn->prepare("SELECT SUM(sk_tot) AS total_amount FROM exam WHERE sk_Id = ? AND date >= ?");
                $sumStmt->bind_param('ss', $row['sk_Id'], $dateFrom); // Bind both parameters
                $sumStmt->execute();
    ...
    $totalRowNumber = $rowIndex + 1; // Assuming you have a total row
        ...
        header('Content-Disposition: attachment;filename="skilled_assistant_bank.xlsx"');
