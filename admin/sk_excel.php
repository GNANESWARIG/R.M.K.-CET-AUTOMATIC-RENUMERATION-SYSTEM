...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty']) && $_POST['faculty'] === 'skilled_assistant') {
        ...
        $sheet->setCellValue('G6', 'Name of the Skilled Assistent');
        ...
        $sheet->getColumnDimension('G')->setWidth(23); // Increased width for College Name
    ...
    $totalAmount = 0;
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sheet->setCellValue('G' . $rowNumber, $row['sk_name']);
            $sheet->setCellValue('H' . $rowNumber, $row['students']);
            $sheet->setCellValue('I' . $rowNumber, $row['sk_renum']);
            $totalCandidates += $row['students'];
            $totalAmount += $row['sk_renum'];
        ...
        header('Content-Disposition: attachment;filename="exam_details_skill.xlsx"');
