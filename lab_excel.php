...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty']) && $_POST['faculty'] === 'lab_technician') {
        ...
        $sheet->setCellValue('G6', 'Name of the Lab Technician');
    ...
    $totalAmount = 0;
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sheet->setCellValue('G' . $rowNumber, $row['lab_name']);
            $sheet->setCellValue('H' . $rowNumber, $row['students']);
            $sheet->setCellValue('I' . $rowNumber, $row['lab_renum']);
            $totalCandidates += $row['students'];
            $totalAmount += $row['lab_renum'];
        ...
        header('Content-Disposition: attachment;filename="exam_details_labTech.xlsx"');
