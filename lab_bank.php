...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty'])&& $_POST['faculty'] === 'lab_technician') {
        ...
        $sql = "SELECT faculty_Id,acc_name,REPLACE(acc_no, ',', '') AS acc_no FROM internals WHERE fac_type='lab_technician'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sumSql = "SELECT SUM(lab_tot) AS total_amount FROM exam WHERE lab_Id= '{$row['faculty_Id']}'";
