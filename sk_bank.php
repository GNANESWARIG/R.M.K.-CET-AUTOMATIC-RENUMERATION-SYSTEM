...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST['faculty'])&& $_POST['faculty'] === 'skilled_assistant') {
        ...
        $sql = "SELECT faculty_Id,acc_name,REPLACE(acc_no, ',', '') AS acc_no FROM internals WHERE fac_type='skilled_assistant'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sheet->setCellValue('B' . $rowIndex, $row['faculty_Id']);
                $sheet->setCellValue('C' . $rowIndex, $row['acc_name']);
                $sheet->setCellValueExplicit('D' . $rowIndex, $row['acc_no'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sumSql = "SELECT SUM(sk_tot) AS total_amount FROM exam WHERE sk_Id= '{$row['faculty_Id']}'";
